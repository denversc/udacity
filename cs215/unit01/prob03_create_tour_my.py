...
def create_tour_iter(nodes):
    ...
    for node in nodes:
        if first_node is None:
            ...
            last_node = node
    yield (node, first_node)

def create_tour(nodes):
    return [x for x in create_tour_iter(nodes)]
...
class Test(unittest.TestCase):
    def test1(self):
        self.do_test([100])
    def test2(self):
        self.do_test([100, 200])
    def test3(self):
        self.do_test(['a', 'b', 'c'])
    def test6(self):
        self.do_test([20, 21, 22, 23, 24, 25])
    def do_test(self, nodes):
        tour = create_tour(nodes)
        self.assertTrue(is_eulerian_tour(nodes, tour))
